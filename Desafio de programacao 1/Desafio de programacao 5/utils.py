...
def add_layer_with_omission(patches, colors, size=(24, 24),
    ...
    for ind in range(this_num):
        ...
        if omit:
            patches.append(
                Circle(loc_start + ind * loc_diff + np.array(size) / 2, 1))
        else:
...
def cria_imagem(model):
    ...
    for ind in range(len(model.size_list)-1,-1,-1):
        if flag_omit:
            add_layer_with_omission(patches, colors, size=model.size_list[ind],
            ...
            add_layer(patches, colors, size=model.size_list[ind],
                      num=num_show_list[ind],
                      top_left=top_left_list[ind], loc_diff=loc_diff_list[ind])
        label(top_left_list[ind], text_list[ind] + '\n{}@{}x{}'.format(
            model.num_list[ind], model.size_list[ind][0], model.size_list[ind][1]), xy_off=[0,30])
    ...
    for ind in range(len(model.patch_size_list)):
        add_mapping(
            ...
            list([0.25*x for x in model.size_list[ind]]), ind,
            top_left_list, loc_diff_list, num_show_list, model.size_list)
        label(top_left_list[ind], text_list[ind] + '\n{}x{}'.format(
            model.patch_size_list[ind][0], model.patch_size_list[ind][1]), xy_off=[350, -350]
        )
    ...
    num_show_list = list(map(min, model.num_fc_list, [NumFcMax] * len(model.num_fc_list)))
    x_diff_list = [sum(x_diff_list) + layer_width] + [layer_width]*(len(model.num_fc_list)-1)
    top_left_list = np.c_[np.cumsum(x_diff_list), np.zeros(len(x_diff_list))]
    ...
    for ind in range(len(size_list)):
        if flag_omit:
            ...
            add_layer(patches, colors, size=size_list[ind],
                      ...
                      loc_diff=loc_diff_list[ind])
        label(top_left_list[ind], text_list[ind] + '\n{}'.format(
            model.num_fc_list[ind]), xy_off=[-10, 20])
